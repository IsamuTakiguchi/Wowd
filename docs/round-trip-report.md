...
いま手元にあるフィクスチャはすべてこちらで生成したもので、
Word 本体が書く XML とは細部が違う。実際の Word で作った `.docx` を
`tests/fixtures/docx/real-*.docx` に置けば既存のテストが自動的に拾う。
手順は `docs/word-verification.md` の「逆方向の確認」にある。
