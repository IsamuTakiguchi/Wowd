...
| 表 | 読み書きと表示は対応済み、挿入・編集 UI は未実装 |
| 画像 | **保持のみ** (原文をそのまま書き戻す)。表示・挿入は未実装 |
| 目次 | フィールドとして保持。生成・更新は未実装 |
| 変更履歴 (w:ins / w:del) | 読み書きは対応済み、校閲 UI は未実装 |
| コメント | 範囲は読み書き対応。**本文は保持のみ** (comments.xml は未解析) |
...
加えて、エディタを通した経路 (`tests/unit/schemaCoverage.test.ts`) も検証している。
モデル層のテストだけでは「ProseMirror のスキーマに無いノードが
読み込み時に捨てられる」種類のバグを検出できないため。実際に一度これで
表・画像・ルビ・フィールドが失われていた。
